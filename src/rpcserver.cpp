...
{
    CBigNum bnAmount;
    bnAmount.setuint256(amount);
    std::string rawAmount;
    rawAmount=bnAmount.ToString();
    if(rawAmount.length()<=8){
        for(int i=0;i<16-rawAmount.length();i++){
           rawAmount+="0";
        }
    }
    return rawAmount.insert(rawAmount.length()-8,".");
}
